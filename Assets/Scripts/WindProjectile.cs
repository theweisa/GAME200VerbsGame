...
public class WindProjectile : BaseDamageSource
{
    ...
    public override void InitDamageSource(BaseDamageable damageable, Vector2 dir)
    {
        ...
        LeanTween.scale(gameObject, transform.localScale*1.5f, lifetime).setEaseOutQuart();
    }

    public void SlowProjectile() {
        float newMagnitude = rb.velocity.magnitude * Global.easeInQuart(lifetimeTimer/lifetime);
        rb.velocity = Vector2.ClampMagnitude(rb.velocity, newMagnitude);
    }
